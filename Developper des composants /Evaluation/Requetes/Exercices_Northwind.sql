...
Je transforme en entier en retirant à la moyenne
le reste de sa division par 1
Utilisation de SUBSTRING_INDEX dans Exercice_Procedures */
SELECT(AVG(DATEDIFF(orders.`ShippedDate`, orders.`OrderDate`)) -
			(AVG(DATEDIFF(orders.`ShippedDate`, orders.`OrderDate`)) % 1))
			AS 'Delais Moyen'
FROM orders;
